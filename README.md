...
- **多种输出模式**: 文本摘要、基于 AstrBot T2I 的长图渲染、TXT 文件、合并转发
...
| `/oc-shell <命令>` | 执行原生 Shell 命令（仅本地模式可用，沿用当前会话工作目录） | `/oc-shell dir` |
...
- `ai_summary`: AI 智能摘要
- `txt_file`: 生成 TXT 文件
- `long_image`: 先由插件完成 Markdown 转 HTML 和模板填充，再调用 AstrBot 原生 T2I 接口渲染长图
...
- `/oc-shell` 仅在 `local` 模式下可用。
- 命令会在当前用户会话的工作目录中执行，与 `/oc` 的工作目录保持一致。
- 即使你关闭了 `full_text`、或把 `txt_file` / `long_image` 都设置成仅长文本触发，短命令输出也会保底直接回显，避免出现“无符合条件的输出”。
