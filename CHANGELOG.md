...
- 长文本图片输出改为直接调用 AstrBot 原生 T2I 自定义模板渲染接口，Markdown 转换和 HTML 模板填充仍由插件自身处理。
- 修复 `/oc-shell` 的执行上下文问题：Shell 命令现在会复用当前会话的工作目录和环境变量，不再脱离会话目录执行。
- 修复 `/oc-shell` 在部分输出配置下执行成功却回落为“无符合条件的输出”的问题；短输出现在会自动回退为直接文本回显。
- 修复空输出分支返回结构不一致的问题，避免发送计划在边界场景下出现异常。
