...
class OpenCodePlugin(Star):
    ...
    def _parse_send_page_query(self, arg_text: str) -> int | None:
        match = re.fullmatch(r"--page\s+(\d+)", arg_text.strip())
        if not match:
            return None
        return int(match.group(1))

    def _parse_send_find_query(self, arg_text: str) -> str | None:
    ...
    async def oc_shell(self, event: AstrMessageEvent, cmd: str = ""):
        ...
        if self.security.is_destructive(actual_cmd):
            ...
            result = await self.executor.exec_shell_cmd(actual_cmd, session=session)
            send_plan = await self.output_proc.parse_output_plan(
                result, event, session, force_text_fallback=True
            )
            for idx, components in enumerate(send_plan):
                if idx > 0:
                    await asyncio.sleep(self.output_proc.next_send_delay())
