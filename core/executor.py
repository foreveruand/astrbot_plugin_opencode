...
class CommandExecutor:
    ...
    async def run_opencode(self, message: str, session: OpenCodeSession) -> str:
        ...
        opencode_extra_args = self.config.get("basic_config", {}).get(
            "opencode_extra_args", ""
        )
        opencode_extra_args_list = opencode_extra_args.split(" ")
        ...
        # 构建命令参数
        cmd_args = [opencode_path, "run", "--format", "json", *opencode_extra_args_list]
    ...
    async def exec_shell_cmd(
        self, cmd: str, session: OpenCodeSession | None = None
    ) -> str:
        """执行 Shell 命令"""
        if self.is_remote_mode():
        ...
        try:
            ...
            process = await asyncio.create_subprocess_shell(
                ...
                stdout=asyncio.subprocess.PIPE,
                stderr=asyncio.subprocess.PIPE,
                cwd=session.work_dir if session else None,
